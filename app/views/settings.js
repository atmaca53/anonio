...
export class SettingsView extends PureComponent<Props, State> {
  ...
  backupWalletDat = async () => {
    const backupFileName = `anon-wallet-backup-${dateFns.format(
      new Date(),
      'YYYY-MM-DD-mm-ss',
    )}.dat`;
    ...
    electron.remote.dialog.showSaveDialog(
      ...
      async (pathToSave: string) => {
        ...
        const anonDir = isTestnet() ? path.join(WALLET_DIR, 'testnet4') : WALLET_DIR;
        const walletDatPath = `${anonDir}/wallet.dat`;
