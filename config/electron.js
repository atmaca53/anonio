...
import runDaemon from './daemon/anond-child-process';
import { log as anonLog, cleanLogs } from './daemon/logger';
...
app.on('ready', async () => {
  ...
  if (process.env.NODE_ENV === 'test') {
    anonLog('Not running daemon, please run the mock API');
    return;
  }
  ...
  runDaemon()
    .then((proc) => {
      if (proc) {
        anonLog(`Anon Daemon running. PID: ${proc.pid}`);
        anonDaemon = proc;
      }
    })
    .catch(anonLog);
...
app.on('before-quit', () => {
  if (anonDaemon) {
    anonLog('Graceful shutdown Anon Daemon, this may take a few seconds.');
    anonDaemon.kill('SIGINT');
  }
});
